#!/usr/bin/env python3
"""Run the four-condition MaleVis deduplication study with repeated seeds.

Conditions
----------
full26
    Conventional pooled 70/20/10 image split with all 26 classes.
full23 (stored as ``full`` for compatibility with completed runs)
    The same conventional split restricted to the 23 classes retained by the
    consensus-clean protocol.
clean_consensus23 (stored as ``clean_consensus``)
    One representative per exact (aHash, dHash, pHash) tuple, followed by a
    70/20/10 split.
random_subsample23
    A random image-level subset of Full-23 with exactly the same class counts
    and partition counts as Clean-Consensus-23. Duplicates are not removed.

The script can reuse completed Full-23 and Clean-Consensus-23 runs in the
output produced by ``train_dedup_before_split.py``.  It never overwrites
changed manifests or completed artifacts.
"""
import argparse
import csv
import hashlib
...
def summarize_results(output):
    ...
    if completed:
        audit.write_csv(output / "four_condition_results.csv", [
            {k: v for k, v in row.items() if k != "artifacts_sha256"}
            for row in completed
        ])

    # Identical 23-class architecture and seed must start from identical weights.
    for seed in sorted({r["seed"] for r in completed}):
        rows = [r for r in completed
                if r["seed"] == seed and r["condition"] in
                {"full", "clean_consensus", "random_subsample23"}]
        if len(rows) > 1 and len({r["initial_state_sha256"] for r in rows}) != 1:
            raise ValueError(f"23-class initialization differs for seed {seed}.")
    ...
    for seed in sorted({r["seed"] for r in completed}):
        ...
        for metric in ("accuracy", "macro_f1", "macro_recall"):
            row[f"full23_minus_clean_{metric}_pp"] = 100 * (
                by["full"][metric] - by["clean_consensus"][metric])
            row[f"full23_minus_subsample_{metric}_pp"] = 100 * (
                by["full"][metric] - by["random_subsample23"][metric])
            row[f"subsample_minus_clean_{metric}_pp"] = 100 * (
                by["random_subsample23"][metric] - by["clean_consensus"][metric])
            row[f"full26_minus_full23_{metric}_pp"] = 100 * (
                by["full26"][metric] - by["full"][metric])
        contrasts.append(row)
    if contrasts:
        audit.write_csv(output / "four_condition_seed_contrasts.csv", contrasts)
    ...
    if metric_rows:
        audit.write_csv(output / "four_condition_metric_summary.csv", metric_rows)
    ...
    if contrasts:
        ...
        audit.write_csv(output / "four_condition_contrast_summary.csv", contrast_rows)
...
def main():
    ...
    full26, classes26, full_meta = load_protocol(args.full_protocol_dir)
    clean23, classes23, clean_meta = load_protocol(
        args.clean_protocol_dir, expected_rule="hash_consensus"
    )
    if not set(classes23) < set(classes26):
        raise ValueError("Expected Clean-Consensus to contain a strict subset of Full classes.")
    full23 = {
        p: [r for r in full26[p] if r["class_label"] in set(classes23)]
        for p in PARTITIONS
    }
    random23 = make_random_subsample(
        full26, clean23, args.subsample_seed, args.subsample_split_seed
    )
    for p in PARTITIONS:
        if Counter(r["class_label"] for r in random23[p]) != Counter(
                r["class_label"] for r in clean23[p]):
            raise AssertionError(f"Random and clean class counts differ in {p}.")

    conditions = {
        "full26": ("full26", full26, classes26),
        "full": ("full", full23, classes23),
        "clean_consensus": ("clean_consensus", clean23, classes23),
        "random_subsample23": ("random_subsample23", random23, classes23),
    }
    ...
    freeze_csv(args.output / "four_condition_dataset_summary.csv", summaries)
    freeze_csv(args.output / "four_condition_class_counts.csv",
    ...
    study = {
        ...
        "subsample_definition": (
            "Class-size- and partition-size-matched individual-image sample from Full-23; "
        ...
        "interpretation": {
            "full26": "contextual 26-class benchmark",
            "full23_minus_clean": "total benchmark-protocol difference",
            "full23_minus_subsample": "sample-quantity and evaluation-population difference",
            "subsample_minus_clean": (
                "size- and class-count-adjusted consensus-deduplication difference"
            ),
        },
    }
    freeze_json(args.output / "four_condition_study.json", study)
    print("Frozen four-condition study:", json.dumps(summaries, indent=2), flush=True)
    ...
    # Preserve compatibility with already completed Full-23/Clean-Consensus runs.
    ...
    if paired_experiment_path.exists():
        ...
        if paired_experiment.get("classes") != classes23:
        ...
        paired_experiment = common_experiment(
            args, classes23, {"full": full_meta, "clean_consensus": clean_meta},
            "paired_23_class_conditions", tf, keras
        )
    ...
    experiments = {
        ...
        "full26": common_experiment(
            args, classes26, full_meta, "full26", tf, keras),
        "random_subsample23": common_experiment(
            args, classes23, {
                "definition": study["subsample_definition"],
                "selection_seed": args.subsample_seed,
                "split_seed": args.subsample_split_seed,
            }, "random_subsample23", tf, keras),
    }
    freeze_json(args.output / "experiment_full26.json", experiments["full26"])
    freeze_json(args.output / "experiment_random_subsample23.json",
                experiments["random_subsample23"])
    ...
    for seed in args.seeds:
        for condition in ORDER:
            ...
            print(f"Four-condition study: {DISPLAY[condition]}, seed {seed}", flush=True)
            corrected.run_one(

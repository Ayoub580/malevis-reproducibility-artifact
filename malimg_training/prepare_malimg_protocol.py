#!/usr/bin/env python3
"""Freeze Full and consensus-clean Malimg protocols before training.

The input is the completed external audit. Every mixed-label consensus group is
excluded. Full uses an individual-image 70/20/10 split over the remaining
...
def prepare(args: argparse.Namespace) -> None:
    ...
    if metadata.get("status") != "completed" or metadata.get("layout") != "class_folders":
    ...
    excluded = sorted({row["class_label"] for row in rows} - set(eligible))
    ...
    common = {
        "dataset": "Malimg local image snapshot",
    ...
    write_protocol(output / "full_safe_23", full_manifest, {
        **common,
        "rule": "individual_image_split_after_excluding_mixed_label_consensus_groups",
        "interpretation": "Full source condition; the trainer restricts it to the clean 20-class label space.",
    })
    write_protocol(output / "clean_consensus_20", clean_manifest, {
        **common,
        "rule": "hash_consensus",
    ...
    summary = {
        ...
        "mixed_label_groups_excluded": len(mixed_values),
        "mixed_label_images_excluded": len(rows) - len(safe),
        "protocol_directories": {"full": "full_safe_23", "clean": "clean_consensus_20"},
    }

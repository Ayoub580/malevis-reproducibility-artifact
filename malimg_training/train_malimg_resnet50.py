#!/usr/bin/env python3
"""Train ResNet50 for the Malimg redundancy experiment.

Conditions
----------
full
    Conventional pooled 70/20/10 image split restricted to the 20 eligible
...
clean_consensus
    ...
    70/20/10 split.
random_subsample20
    A random image-level subset of Full-20 with exactly the same class and
    partition counts as Clean-Consensus-20. Consensus duplicates are retained
    when selected.
...
DISPLAY = {
    "full": "Full-20",
    "clean_consensus": "Clean-Consensus-20",
    "random_subsample20": "Random-Subsample-20",
}
ORDER = tuple(DISPLAY)
    ...
    for seed in sorted({r["seed"] for r in completed}):
        ...
        for metric in ("accuracy", "macro_f1", "macro_recall"):
            ...
            row[f"full_minus_subsample_{metric}_pp"] = 100 * (
                by["full"][metric] - by["random_subsample20"][metric])
            row[f"subsample_minus_clean_{metric}_pp"] = 100 * (
                by["random_subsample20"][metric] - by["clean_consensus"][metric])
...
def main():
    ...
    full23, classes23, full_meta = load_protocol(args.full_protocol_dir)
    clean20, classes20, clean_meta = load_protocol(
        args.clean_protocol_dir, expected_rule="hash_consensus"
    )
    if not set(classes20) < set(classes23):
        raise ValueError("Expected Clean-Consensus to contain a strict subset of Full classes.")
    full20 = {
        p: [r for r in full23[p] if r["class_label"] in set(classes20)]
        for p in PARTITIONS
    }
    random20 = make_random_subsample(
        full23, clean20, args.subsample_seed, args.subsample_split_seed
    )
    for p in PARTITIONS:
        if Counter(r["class_label"] for r in random20[p]) != Counter(
                r["class_label"] for r in clean20[p]):
            raise AssertionError(f"Random and clean class counts differ in {p}.")

    conditions = {
        "full": ("full", full20, classes20),
        "clean_consensus": ("clean_consensus", clean20, classes20),
        "random_subsample20": ("random_subsample20", random20, classes20),
    }
    ...
    study = {
        ...
        "subsample_definition": (
            "Class-size- and partition-size-matched individual-image sample from Full-20; "
    ...
    print("Frozen Malimg study:", json.dumps(summaries, indent=2), flush=True)
    ...
    if paired_experiment_path.exists():
        ...
        if paired_experiment.get("classes") != classes20:
        ...
        paired_experiment = common_experiment(
            args, classes20, {"full": full_meta, "clean_consensus": clean_meta},
            "paired_20_class_conditions", tf, keras
        )
    ...
    experiments = {
        ...
        "random_subsample20": common_experiment(
            args, classes20, {
                "definition": study["subsample_definition"],
                "selection_seed": args.subsample_seed,
                "split_seed": args.subsample_split_seed,
            }, "random_subsample20", tf, keras),
    }
    freeze_json(args.output / "experiment_random_subsample20.json",
                experiments["random_subsample20"])
    ...
    for seed in args.seeds:
        for condition in ORDER:
            ...
            print(f"Malimg study: {DISPLAY[condition]}, seed {seed}", flush=True)
            corrected.run_one(
